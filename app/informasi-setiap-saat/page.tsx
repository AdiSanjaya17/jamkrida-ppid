...
export const metadata = { title: "Informasi Setiap Saat" };

export default async function InformasiSetiapSaatPage() {
  const [navItems, settings, infos] = await Promise.all([
    ...
    prisma.publicInformation.findMany({
      where: { status: "PUBLISHED", category: "SETIAP_SAAT" },
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <PageBanner title="Informasi Setiap Saat" breadcrumb={[{ label: "Beranda", href: "/" }, { label: "Informasi Setiap Saat" }]} />
        ...
        <div className="mx-auto max-w-4xl px-4 py-12 lg:px-8">
          ...
          {infos.length === 0 ? (
            <div className="mt-10 rounded-lg border border-dashed border-neutral-300 bg-neutral-50 p-10 text-center">
              <p className="text-neutral-500">Belum ada informasi setiap saat</p>
            </div>
          ) : (
