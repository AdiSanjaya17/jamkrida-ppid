...
export const metadata = { title: "Penghargaan" };
...
export default async function PenghargaanPage() {
  const [navItems, settings, news] = await Promise.all([
    ...
    prisma.news.findMany({
      where: { status: "PUBLISHED", category: "Penghargaan" },
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <PageBanner title="Penghargaan" breadcrumb={[{ label: "Beranda", href: "/" }, { label: "Penghargaan" }]} />
        ...
        <div className="mx-auto max-w-4xl px-4 py-12 lg:px-8">
          ...
          {news.length === 0 ? (
            <div className="mt-10 rounded-lg border border-dashed border-neutral-300 bg-neutral-50 p-10 text-center">
              <p className="text-neutral-500">Belum ada penghargaan yang dipublikasikan</p>
            </div>
          ) : (
                ...
                <article key={item.id} className="rounded-lg border border-neutral-200 bg-white p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <span className="inline-block rounded-full bg-yellow-100 px-3 py-1 text-xs font-semibold text-yellow-800 mb-3">
                        Penghargaan
                      </span>
