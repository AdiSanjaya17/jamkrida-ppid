...
export const metadata = { title: "LHKPN" };
...
export default async function LHKPNPage() {
  const [navItems, settings, documents] = await Promise.all([
    ...
    prisma.document.findMany({
      where: { status: "PUBLISHED", category: "LHKPN" },
      orderBy: { year: "desc" },
    }),
  ]);
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <PageBanner title="LHKPN" breadcrumb={[{ label: "Beranda", href: "/" }, { label: "LHKPN" }]} />

      <main className="flex-1">
        <div className="mx-auto max-w-4xl px-4 py-12 lg:px-8">

          <div className="mt-10 rounded-lg border border-brand/30 bg-brand/5 p-6 mb-8">
            <h2 className="font-semibold text-neutral-900">Tentang LHKPN</h2>
            <p className="mt-3 text-sm text-neutral-700">
              Laporan Harta Kekayaan Pejabat Negara (LHKPN) adalah laporan tentang kekayaan pejabat negara yang dibuat sebagai bagian dari upaya pencegahan dan pemberantasan korupsi di Indonesia.
            </p>
          </div>

          {documents.length === 0 ? (
            <div className="mt-10 rounded-lg border border-dashed border-neutral-300 bg-neutral-50 p-10 text-center">
              <p className="text-neutral-500">Belum ada LHKPN yang dipublikasikan</p>
            </div>
          ) : (
