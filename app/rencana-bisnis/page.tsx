...
export const metadata = { title: "Rencana Bisnis" };

export default async function RecanaBisnisPage() {
  const [navItems, settings, documents] = await Promise.all([
    ...
    prisma.document.findMany({
      where: { status: "PUBLISHED", category: "Rencana Bisnis" },
      orderBy: { year: "desc" },
    }),
  ]);
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <PageBanner title="Rencana Bisnis" breadcrumb={[{ label: "Beranda", href: "/" }, { label: "Rencana Bisnis" }]} />
        ...
        <div className="mx-auto max-w-4xl px-4 py-12 lg:px-8">
          ...
          {documents.length === 0 ? (
            <div className="mt-10 rounded-lg border border-dashed border-neutral-300 bg-neutral-50 p-10 text-center">
              <p className="text-neutral-500">Belum ada rencana bisnis</p>
            </div>
          ) : (
